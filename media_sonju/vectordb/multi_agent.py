...
def tech_chat_llm(state: SelfRAGState) -> SelfRAGState:
...
def customer_chat_llm(state: SelfRAGState) -> SelfRAGState:
    print(f"{state.get("domain")} AI 챗봇 실행")
    context = state.get("context")
    question = state.get("question")
    prompt_template = f"""
    ...
    chat_prompt = ChatPromptTemplate.from_messages([
        SystemMessage(
            content=(
                "당신은 계약, 요금 등에 관한 고객 서비스 상담 AI 챗봇입니다. 항상 친절하고 알기 쉽게 고객 서비스에 대해 안내해주세요."
            )
        ),
        HumanMessagePromptTemplate.from_template(prompt_template),
        ])
