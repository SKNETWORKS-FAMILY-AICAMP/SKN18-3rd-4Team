"""
SK매직 FAQ 크롤링 스크립트
- text 컬럼: 순수 텍스트만 (표와 이미지 제외)
- table_markdown 컬럼:표 데이터를 [표1], [표2].. Markdown 형식으로 저장
- images 컬럼: 이미지 URL을 '|' 구분자로 저장
"""
import csv
import re
import os
...
def extract_content(html):
    """HTML에서 텍스트와 이미지 추출 (표와 이미지는 별도 컬럼에 저장)"""
...
def crawl_faq(main_cat, cddtlid, use_bracket=False):
    ...
    try:
        with sync_playwright() as p:
            ...
            # 저장 폴더 생성
            output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'skmagic_faq_split_cols')
