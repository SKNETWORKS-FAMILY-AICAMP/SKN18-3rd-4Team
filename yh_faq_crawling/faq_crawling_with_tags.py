"""
SK매직 FAQ 크롤링 스크립트 (태그 버전)
- 원본 텍스트 위치에 맞는 [표1], [이미지1] 태그 삽입
- 표 데이터는 Markdown 형식으로 table_markdown 컬럼에 저장
- 이미지 URL은 '|' 구분자로 images 컬럼에 저장
"""
import csv
import re
import os
...
def extract_content(html):
    """HTML에서 텍스트와 이미지 추출 (DOM 순서대로 위치에 태그 삽입)"""
    ...
    # DOM 순서대로 순회하면서 표와 이미지를 태그로 치환
    def process_element(element):
        nonlocal table_num, img_num

        # 자식 요소들을 순회 (리스트로 변환하여 순서 보장)
        ...
        for child in children:
            if child.name == 'table':
                if has_border(child):
                    ...
                    if rows:
                        ...
                        if md:
                            ...
                            tag.string = f' [표{table_num}] '
                            child.replace_with(tag)
                            table_num += 1
                            all_images.extend(table_imgs)
                        else:
                            child.decompose()
                    else:
                        child.decompose()
                ...
                if src and not src.lower().endswith('.mp4') and src.strip():
                    img_url = normalize_url(src)
                    all_images.append(img_url)
                    # 이미지를 태그로 치환
                    tag = soup.new_tag('span')
                    tag.string = f' [이미지{img_num}] '
                    child.replace_with(tag)
                    img_num += 1
    ...
    return text, '\n\n'.join(table_markdowns), '|'.join(all_images) if all_images else "없음"
...
def crawl_faq(main_cat, cddtlid, use_bracket=False):
    ...
    try:
        with sync_playwright() as p:
            ...
            # 저장 폴더 생성
            output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'skmagic_faq_with_tags')
